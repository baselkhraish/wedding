...
@section('content')
    ...
    <div class="container-fluid bg-secondary mb-5">
        <div class="d-flex flex-column align-items-center justify-content-center" style="min-height: 300px">
            <h1 class="font-weight-semi-bold text-uppercase mb-3">{{ $vendor->name }}</h1>
        ...
        <div class="row px-xl-5">
            ...
            <div class="col-lg-12 col-md-12">
                <div class="row pb-3">
                    ...
                    @foreach ($vendor->product as $item)
                        <div class="col-lg-3 col-md-6 col-sm-12 pb-1">
                            <div class="card product-item border-0 mb-4">
                                <div class="card-header product-img position-relative overflow-hidden bg-transparent border p-0">
                                    <a href="{{ route('product_details',$item->id) }}"><img class="img-fluid w-100" src="{{ asset('uploads/images/product/'.$item->image) }}" alt="{{ $item->name }}" title="{{ $item->name }}" style="height: 300px; object-fit: contain" ></a>
                                </div>
                                <div class="card-body border-right border-left text-center p-0 pt-4 pb-3">
                                    <div class="d-flex justify-content-between align-items-center pr-5 pl-5 mb-3">
                                        <div><span>{{ $item->vendor->name }}</span></div>
                                        <div><a href="{{ route('category.product',$item->vendor->category->id) }}"><span>{{ $item->vendor->category->name }}</span></a></div>
                                    </div>
